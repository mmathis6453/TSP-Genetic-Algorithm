    ...
    extends JFrame
{
   ...
   * @param keep_top - How many of the top performing paths are crossed and mutated to fill the population of the next
   *        generation
  ...
  private boolean validateTextFields()
  {
    ...
    if (!allowTwins.isSelected()) {
      ...
      if (popSizeBigInt.compareTo(uniquePaths) > 0) {
        JOptionPane.showMessageDialog(
            this,
            "Allow Twins is selected.\nThere population is too large to hold all unique paths.\n" + "There are only "
                + uniquePaths.toString() + " unique paths for " + Integer.toString(pointList.size()) + " points");
        return false;
      }
    }
    ...
    if (popInt >= cores) {
      ...
      JLabel message = new JLabel(
          "Running with as many or more populations than you have available cores (" + Integer.toString(cores)
